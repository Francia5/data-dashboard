...
for(var i = 0; i < data.SCL['2017-2'].students.length; i++){
    ...
    nameStudent += '<li>' + ("Nombre de Studiante: " + (data.SCL['2017-2'].students[i].name)) + '</li>';    
    // break;
...
var totalStudents = data.SCL['2017-2']; 
totalStudents = studentsInscribed(totalStudents);
document.getElementById('students-inscribed-SCL2016').innerHTML = ("Cantidad de estudiantes inscritas SCL 2017 2: " + totalStudents);
...
// alumanas activas 
var totalTOTALSCL = data.SCL['2017-2'];
totalTOTALSCL = activeStudentsSCL(totalTOTALSCL);
document.getElementById('students-active-SCL2016').innerHTML = ("Cantidad de estudiantes activas SCL 2017 2: " + totalTOTALSCL);
...
document.getElementById('students-desertion-SCL2016').innerHTML = ("Cantidad de estudiantes desertoras SCL 2017 2: " + dropoutSCL);

for(var i = 0; i < data.SCL['2017-2'].students.length; i++){
    if(data.SCL['2017-2'].students[i].active == false){
        deserter += '<li>' + (data.SCL['2017-2'].students[i].name) + '</li>';
    }
...
for(var i = 0; i < data.SCL['2017-2'].students.length; i++){
    if(data.SCL['2017-2'].students[i].active == true){
        actives += '<li>' + (data.SCL['2017-2'].students[i].name) + '</li>';
    }
...
// var hse = '';
// for(var j = 0; j < data.SCL['2017-2'].students[i].sprints.length; j++){    
//     tech += '<li>' + ("Sprint: " + " : " + (data.SCL['2017-2'].students[i].sprints[j].number) + " = " + (data.SCL['2017-2'].students[i].sprints[j].score.tech)); + '</li>';
// }
// document.getElementById('activeTECH').innerHTML = tech;


// for(var j = 0; j < data.SCL['2017-2'].students[i].sprints.length; j++){    
//     hse += '<li>' + ("Sprint: " + " : " + (data.SCL['2017-2'].students[i].sprints[j].number) + " = " + (data.SCL['2017-2'].students[i].sprints[j].score.hse)); + '</li>';
//     // break;    
// }
...
//alumnas inscritas (students)
var studentsInscribed = function students(data) {
    var studentsNumber = data.SCL['2017-2'].students.length;
    return studentsNumber;
}
...
for(var i = 0; i < data.SCL['2017-2'].students.length; i++){
    ...
    if(data.SCL['2017-2'].students[i].active == true){
        for(var j = 0; j < data.SCL['2017-2'].students[i].sprints.length; j++){    
                points += data.SCL['2017-2'].students[i].sprints[j].score.tech;
                points += data.SCL['2017-2'].students[i].sprints[j].score.hse;
        }
        if(points >= 8400){
            quantity++;
        }
